        ...
        <div class="col-md-12 stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <form id="createForm" action="{{ route('challenge.update', ['challenge' => $challenge]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="row">
{{--                            <div class="col-sm-6">--}}
                                ...
                                <div class="mb-3 innerDashboard-inputs">
                                    ...
                                    <select class="form-control" name="price">
                                        @foreach($prices as $price)
                                            <option value="{{$price->id}}" {{$challenge->price_id == $price->id ? 'selected':''}}>{{$price->price}}</option>
                                        @endforeach
                                ...
                                <div class="mb-3 innerDashboard-inputs">
                                    ...
                                    <select name="createdBy" class="form-control">
                                        @foreach($users as $user)
                                            <option value="{{$user->id}}" {{$challenge->created_by == $user->id ? 'selected':''}}>{{$user->full_name.'('.$user->nick_name.')'}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div><!-- Col -->
                            ...
                            <div class="col-sm-6">
                                <div class="mb-3 innerDashboard-inputs">
                                    ...
                                    <select name="status" class="form-control">
                                        <option value="0" {{$challenge->status == 0 ? 'selected':''}}>{{ucfirst(__('options.disapproved'))}}</option>
                                        <option value="1" {{$challenge->status == 1 ? 'selected':''}}>{{ucfirst(__('options.approved'))}}</option>
                                    </select>
                                </div>
                            </div><!-- Col -->
